...
class EfficientLowNoiseSentimentalNeuralNetwork(object):
    def __init__(self, training_data, validation_data, num_hidden_nodes=10, num_epochs=10, learning_rate=0.1, min_count=10, polar_cutoff=0.1):
    ...
    def train(self):
        ...
        for review in self.training_data.Text:
            indices = set()
        ...
        for epoch in range(self.num_epochs):
            ...
            for row in range(self.training_data.shape[0]):
                ...
                self.forward_backward_propagate(training_data_text[row], self.training_data.Label[row])
                # calculate our speed
                ...
                sys.stdout.write("\rEpoch: "+ str(epoch)
                                 + " Progress: " + str(100 * row/float(self.training_data.shape[0]))[:4] + "%"
            ...
            training_loss = self.run(self.training_data[0:7000], mode="train")
            validation_loss = self.run(self.validation_data,  mode="validate")
    ...
    def pre_process_data(self, training_data):
        ...
        for i in range(self.training_data.shape[0]):
            if(self.training_data.Label[i] == 'POSITIVE'):
                for word in self.training_data.Text[i].split(" "):
                    positive_counts[word] += 1
                    total_counts[word] += 1
            if(self.training_data.Label[i] == 'NEGATIVE'):
                for word in self.training_data.Text[i].split(" "):
